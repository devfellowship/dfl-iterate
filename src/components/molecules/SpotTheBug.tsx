...
export function SpotTheBug({ activity, onSuccess, onError }: SpotTheBugProps) {
  ...
  const challenge = useMemo(() => {
    return generateBugChallenge();
  }, [activity.id]);

  const lines = useMemo(
    () => challenge.code.split(/\r?\n/),
    [challenge],
  );
  ...
  const highlightedLines = useMemo(() => {
    const html = Prism.highlight(
      challenge.code,
      (Prism as unknown as { languages: Record<string, unknown> }).languages.typescript ?? Prism.languages.javascript,
      'typescript',
    );
    return html.split('\n');
  }, [challenge]);
  ...
  return (
    ...
    >
      ...
      <div className="bg-[#050505] rounded-xl overflow-hidden border border-white/5 flex flex-col">
        ...
        <div className="flex-1 px-6 py-6 font-mono text-[13px] leading-relaxed text-zinc-100 overflow-auto">
          <div className="space-y-1">
            {lines.map((line, index) => {
              ...
              const highlighted = highlightedLines[index] ?? line;
              ...
              return (
                ...
                >
                  ...
                  <span className="whitespace-pre text-[13px] text-zinc-100/90" dangerouslySetInnerHTML={{ __html: highlighted }} />
                </div>
              );
            })}
          </div>
